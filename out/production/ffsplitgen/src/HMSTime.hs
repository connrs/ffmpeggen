...
-- Data type
data HMSTime = MkHMSTime { hour :: Int, minute :: Int, second :: Int, millisecond :: Int }
             | EndHMSTime
             | StartHMSTime
...
instance Eq HMSTime where
  ...
  a@MkHMSTime{} == b@MkHMSTime{} =
    and
      [hour a == hour b, minute a == minute b, second a == second b, millisecond a == millisecond b]
  _ == _ = False
...
mkHMSTime h m s ms
  ...
  | validComponents m s ms =
      MkHMSTime { hour = h, minute = m, second = s, millisecond = ms }
