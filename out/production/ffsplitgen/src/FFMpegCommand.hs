...
ffmpegCommandSegment inFile extension commonParams index (start, stop)
  | stop == EndHMSTime = CommandComponent [inParam, commonParams, startParam, outputParam]
  | otherwise = CommandComponent [inParam, commonParams, startParam, stopParam, outputParam]
  where
    inParam = "-i " ++ inFile
